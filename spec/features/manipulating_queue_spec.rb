...
feature "When an admin is signed in," do
    ...
    scenario "if two tickets are present for a given floor, they may admit the first person to the gym and see their entry removed from the queue and the other person's position updated" do
        ...
        expect(page).to have_link('Admit to Top')
        expect(page).not_to have_link('Admit to Bottom')
        click_link 'Admit to Top'
    ...
    scenario "if a singular ticket is present, they may admit that person to the gym, but should expect the admit button for that floor to disappear" do
        admin = User.create!(:email => 'admin@colgate.edu', :admin => true, :password => "colgate13")
        ...
        click_link 'Admit to Top'
        expect(page).not_to have_content('stheisen@colgate.edu')
        expect(page).not_to have_link('Admit to Top')
        expect(page).to have_content("Shelby Theisen was admitted to the top floor of the gym.")
    end
    
    scenario "if tickets are not present, the admit buttons will not appear" do
        admin = User.create!(:email => 'admin@colgate.edu', :admin => true, :password => "colgate13")
        Ticket.queueBottom.clear
        login_as(admin, :scope => :user)
        visit(tickets_path)
